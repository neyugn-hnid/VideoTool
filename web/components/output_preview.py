...
def render_single_output(pixelle_video, video_params):
    ...
    with st.container(border=True):
        ...
        # Premium Generate Button (wrapped in styled container)
        st.markdown('<div class="generate-btn-container">', unsafe_allow_html=True)
        if st.button(tr("btn.generate"), type="primary", use_container_width=True, key="generate_video_btn"):
            ...
            try:
                ...
                info_text = (
                    f"{tr('info.generation_time')} {total_generation_time:.1f}s   "
        ...
        # Close generate button container
...
def render_batch_output(pixelle_video, video_params):
    ...
    with st.container(border=True):
        ...
        if st.button(
            ...
            col2.metric(tr("batch.success"), batch_result["success_count"])
            col3.metric(tr("batch.failed"), batch_result["failed_count"])
            
            # Display total time
            minutes = int(total_time / 60)
            seconds = int(total_time % 60)
            st.caption(f"{tr('batch.total_time')}: {minutes}{tr('batch.minutes')}{seconds}{tr('batch.seconds')}")
